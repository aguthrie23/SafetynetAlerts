...
public class FileLoaderRepository {
	...
	public void runLoad () throws IOException {
        String filePath = "src/main/resources/data.json";
     	...
     	personAny.forEach(a -> personRepository.addPerson(new Person.PersonBuilder().firstName(a.get("firstName").toString())
        ...
        Map<String, FireStations> fireStationMap = new HashMap<>();
     	...
     	fireStationAny.forEach(anyStation -> { 
     		fireStationMap.compute(anyStation.get("station").toString(), 
     			    (k, v) -> v == null ?
     			    		new FireStations(anyStation.get("station").toString()).addAddress(anyStation.get("address").toString()) :
        ...
        fireStations.forEach(fire -> fireStationsRepository.addFireStation(fire));     	
     	...
     	medicalAny.forEach(medRec -> {
     		...
     		med.setMedicationsList(medications.asList());
     		med.setAllergiesList(allergies.asList());
   ...
   for (MedicalRecord medicalRecord : testList) {
	System.out.println(medicalRecord.getFirstName() + "\t" + medicalRecord.getLastName() + "\t" + medicalRecord.getMedicationsList());
}
    }
 }
